...
public final class FormatUtils {
     ...
     * @return
     */
    public static boolean isUserLinkUrl(@NonNull String url) {
        return !TextUtils.isEmpty(url) && url.startsWith(ApiDefine.USER_LINK_URL_PREFIX);
    }

    public static boolean isTopicLinkUrl(@NonNull String url) {
    ...
    public static String renderMarkdown(String text) {
        ...
        //解析@协议
        text = text.replaceAll("@([\\w\\-]+)\\b(?![\\]\\<\\.])",
                "[@$1](" + ApiDefine.USER_PATH_PREFIX + "$1)");
        //解析裸协议
        text = text + " ";
