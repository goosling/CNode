...
import joe.com.cnode.util.Crypto;
...
public class SharedWrapper {
        ...
        if(secretSingleton == null || ivSingleton == null) {
            synchronized (SharedWrapper.class) {
                if(secretSingleton == null) {
                    secretSingleton = Crypto.AES.generateSecret(Digest.SHA256.getRaw(DeviceInfo
    ...
    private String get(@NonNull String key, @NonNull String defValue) {
    ...
    private void set(@NonNull String key, @NonNull String value) {
        ...
        if(value == null) {
            ...
            try{
                target = Base64.encodeToString(Crypto.AES.encrypt(secret, iv, value.getBytes(
                        CHARSET_UTF_8)), Base64.DEFAULT);
            } catch(Crypto.CryptoException e) {
    ...
    public Long gettLong(@NonNull String key, long defValue) {
        String value = get(key, null);
        if(value == null) {
            return defValue;
        } else {
    ...
    public void setObject(@NonNull String key, @NonNull Object value) {
